...
categories.forEach((categorie) => {
  inputSelect.innerHTML += `<option>${categorie}</option>`;
});

function showModal(id) {
  let objeto = response.find((item) => item.id === id);
            ...
            <div class="col-lg-4 col-md-6 mb-4" onclick="showModal(${element.id})">
                <div class="card">
                    <div class="imgOverflow">
                        <img src="${element.image}" class="imgOverflow m-2" alt="${element.title}">
                    </div>
                    <div class="card-body">
                        <h5 class="card-title">${element.title}</h5>
                        <p class="card-title fs-3">${element.price}$</p>
                    </div>
                </div>
            </div>
        `;
...
inputSelect.addEventListener("change", function () {
  let categoriaSeleccionada = inputSelect.value;
  productos.innerHTML = "";
  let responseFiltrada = response.filter(
...
productos.innerHTML = "";
let responseFiltrada = response.filter(
  (product) => product.category === inputSelect.value
);
responseFiltrada.forEach((element) => {
  let card = `
            <div class="col-lg-4 col-md-6 mb-4" onclick="showModal(${element.id})">
